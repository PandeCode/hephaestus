...
impl From<&ClientSpace> for WsMeta {
	fn from(w: &ClientSpace) -> Self {
		Self {
			...
			occupied: !w.is_empty(),
			extent: (0.0, 0.0),
			fg: Color::new_from_hex(0x00000000),
		}
	}
}
...
impl<X: XConn> Widget<X> for Workspaces {
	fn draw(
		...
		for (i, ws) in self.workspaces.iter().enumerate() {
			if ws.occupied || self.focused_ws.iter().any(|t| t == &ws.tag) {
				let (_, bg) = self.ws_colors(&ws.tag, screen, screen_has_focus, ws.occupied);
				let fg = &Color::new_from_hex(TAGS_COLORS[i]);
				if let Some(c) = bg {
					ctx.color(c);
